...
export class AdminReviewsRepository implements IAdminReviewsRepository {
  ...
  async findById(id: string): Promise<AdminReview | null> {
    ...
    return {
      ...
      verifiedPurchase: reviewDoc.verifiedPurchase || false,
  ...
  async getStats(period?: string): Promise<ReviewStats> {
    // Get time filter
    const timeFilter = period !== 'all' ? {
      createdAt: { $gte: new Date(Date.now() - this.getPeriodMilliseconds(period || '30days')) }
    } : {};
  ...
  private getPeriodMilliseconds(period: string): number {
    switch (period) {
      ...
      case '90days': return 90 * 24 * 60 * 60 * 1000;
